  ...
  return (
    <motion.div exit={{ opacity: 0 }} transition={{ duration: 0.4 }} className="fixed inset-0 z-[100] bg-espresso flex flex-col items-center justify-center">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="text-center">
        <span className="font-display text-[30px] font-semibold text-ivory tracking-[-0.02em]">
          Velvet <span className="italic text-champagne">&</span> Fade
        </span>
      </motion.div>
      <div className="mt-8 w-32 h-[2px] bg-ivory/8 rounded-full overflow-hidden">
        <motion.div className="h-full bg-plum rounded-full" initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 0.3, ease: "easeOut" }} />
      </div>
    </motion.div>
  );
}
